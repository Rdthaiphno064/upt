...
if ! command -v sqlite3 >/dev/null; then
    pkg update && pkg upgrade -y && pkg install -y sqlite && pkg install jq >/dev/null
fi
...
force_restart() {
    ...
    am start -n ${pkg}/com.roblox.client.startup.ActivitySplash -d "roblox://placeID=${GAME_ID}" >/dev/null
    sleep 10
    echo "Vào GameID $GAME_ID Cho $pkg"
    am start -n ${pkg}/com.roblox.client.ActivityProtocolLaunch -d "roblox://placeID=${GAME_ID}" >/dev/null
